    ...
    <div className="space-y-6">
      <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-gray-50">
          <h2 className="text-2xl font-black text-gray-900 tracking-tight italic">YOUR LIBRARY</h2>
          <p className="text-gray-400 text-[10px] font-bold uppercase tracking-[0.2em]">Tap an active book to sync progress</p>
        </div>
              ...
              return (
                <button
                  ...
                  className={`group relative flex flex-col text-left transition-all duration-300 ${
                    isCompleted ? 'cursor-default' : 'hover:-translate-y-2'
                  }`}
                >
                  {/* Book Cover Container */}
                  <div className={`relative aspect-[2/3] w-full rounded-2xl overflow-hidden shadow-lg border-4 transition-all ${
                    isSelected ? 'border-purple-500 ring-8 ring-purple-50' : 'border-transparent'
                  }`}>
                    ...
                    {isCompleted && (
                      <div className="absolute top-2 right-2 flex items-center justify-center w-8 h-8 bg-green-500 text-white rounded-full shadow-lg border-2 border-white animate-in zoom-in duration-300">
                  ...
                  <div className="mt-4 px-1">
                    <h3 className="font-black text-gray-900 text-[13px] truncate leading-tight group-hover:text-purple-600 transition-colors uppercase tracking-tight">
                      {reading.title}
                    </h3>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div 
                          className={`h-full transition-all duration-500 ${isCompleted ? 'bg-green-500' : 'bg-purple-600'}`} 
                          style={{ width: `${progress}%` }} 
                        />
                      </div>
                      <span className={`text-[10px] font-black ${isCompleted ? 'text-green-600' : 'text-gray-400'}`}>
      ...
      {selectedReading && (
        ...
        >
          <div 
            className="bg-white rounded-[40px] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300"
            onClick={e => e.stopPropagation()}
          >
            {/* Concept Header */}
            <div className="relative p-8 bg-purple-600 flex items-center gap-4">
              <button 
                onClick={closeUpdateModal}
              ...
              <div className="text-white pr-8">
                ...
                <p className="text-purple-200 text-xs font-bold uppercase tracking-widest">{selectedReading.author}</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-4 rounded-3xl text-center border border-gray-100">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.15em] mb-1">Current Page</p>
                    <input 
                      type="number" 
                      value={newPagesRead}
                      onChange={(e) => {
                      ...
                      className="text-2xl font-black text-purple-600 bg-transparent w-full text-center outline-none"
                    />
                  </div>
                  <div className="bg-gray-50 p-4 rounded-3xl text-center border border-gray-100">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.15em] mb-1">Total Pages</p>
                    <p className="text-2xl font-black text-gray-900">{selectedReading.total_pages}</p>
                  </div>
                </div>
              </div>
              ...
              <button 
                type="submit" 
                disabled={loading}
                className="w-full py-5 bg-purple-600 text-white rounded-[24px] font-black text-lg hover:bg-purple-700 transition-all shadow-xl shadow-purple-100 disabled:bg-gray-200"
              >
